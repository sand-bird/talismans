...
import AVAILABLE_SKILLS from './avail_skills.json'
...
export const loadSaves = (file) => {
  ...
  for (let i = 0; i <= 2; i++) {
    ...
    if (checkSave(file, i)) {
      ...
      let name = file.toString('utf8', nameOffset, nameOffset + NAME_SIZE)
...
export const processDecorations = (charm) => {
  if (charm.decorations && charm.decorations.length === 3) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (decoration && DECORATIONS[decoration]) { filled += DECORATIONS[decoration].slots }
    }
